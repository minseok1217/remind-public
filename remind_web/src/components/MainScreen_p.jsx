...
function MainScreen_p({ currentUser, onViewAllCallHistory }) { // 컴포넌트 이름 변경
  ...
  const [webToken, setWebToken] = useState("토큰 대기 중...");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!token) {
        console.log('웹 토큰 생성 실패');
        return;
      }

      console.log('웹 FCM 토큰:', token);

      setWebToken(token);

      // Firestore 저장
      await setDoc(
        doc(db, 'patients', currentUser.uid),
        {
          web_token: token,
          web_token_updated_at: new Date().toISOString()
        },
        { merge: true }
      );

      console.log('웹 토큰 저장 완료');
    } catch (error) {
      console.error('웹 토큰 저장 실패:', error);
    }
  };
