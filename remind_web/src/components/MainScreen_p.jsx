...
function MainScreen_p({ currentUser, onViewAllCallHistory }) { // 컴포넌트 이름 변경
  ...
  const [isConnectedToGuardian, setIsConnectedToGuardian] = useState(false);
    ...
    if (currentUser) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        if (snapshot.docs.length > 0) {
          ...
          setIsConnectedToGuardian(true);
          setTempCode(null); // 코드가 성공적으로 사용되었으므로 코드와 카운트다운 중지
          setCountdown(0);
          setConnectionMessage('보호자와 연결되었습니다.'); // alert 대신 setConnectionMessage 사용
        } else {
          console.log('[MainScreen_p] 보호자 연결 안됨.');
          setIsConnectedToGuardian(false);
          setConnectionMessage(null); // 연결이 끊어졌을 때 메시지 초기화
        }
      }, (error) => {
